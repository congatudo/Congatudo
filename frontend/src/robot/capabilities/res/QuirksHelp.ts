...
Congatudo aims to be a generic abstraction. That means that it tries to unify vendor-specific commands and concepts
into generic ones so that you don't have to worry about which brand of robot you buy as Congatudo will work
the same on all of them.

Sometimes however there might be things that only one Vendor or even only one model of robot does.
Adding that to the core infrastructure of Congatudo wouldn't make sense as the generic interface would soon become an 
...
**A quirk is a vendor, robot or firmware-specific tunable that doesn't fit into Congatudo (yet?).**
