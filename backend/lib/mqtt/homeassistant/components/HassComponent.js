...
class HassComponent {
     ...
     */
    getAutoconfTopic() {
        const nodeId = this.hass.toHassObjectIdFragment(this.hass.identifier);
        const objectId = this.hass.toHassObjectIdFragment(this.componentId);

        return "homeassistant/" + this.componentType + "/" + nodeId + "/" + objectId + "/config";
    }
